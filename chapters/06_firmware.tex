...
\begin{frame}{Why an MCU at all?}
  \begin{columns}[T,onlytextwidth]
    \column{0.50\textwidth}
      Jetson already runs Linux and ROS.\\
      So why add an STM32F767?

      \vspace{0.4em}
      \textbf{Because control needs a smaller, stricter world.}
      \begin{enumerate}\setlength\itemsep{2pt}
        \item \alert{Determinism.}\;
              PWM, encoders and PID need bounded jitter.
        \item \alert{Separation of concerns.}\;
              If the Jetson crashes, firmware still owns the wheels.
        \item \alert{Safety.}\;
              Motor power is enabled only after firmware bring-up.
      \end{enumerate}

    \column{0.45\textwidth}
      \textbf{F7 hardware budget.}
      \begin{itemize}\setlength\itemsep{1pt}
        \item Cortex-M7 \(@\) \(\sim\!216\)~MHz + FPU.
        \item 4 QEI timers: wheel encoders.
        \item 4 PWM channels: motor driver.
        \item CAN RX ISR + TX task.
        \item I\(^2\)C IMU, optional.
      \end{itemize}

      \vspace{0.4em}
      \footnotesize\textcolor{black!60}{A few hundred lines on HAL \(+\)
      FreeRTOS. Not low-level heroics: just \emph{boring predictability}.}
  \end{columns}
\end{frame}
\end{comment}
% ---------------------------------------------------------------------------- %
...
\begin{frame}{Inner loop \(@\) \(200\)~Hz}
  ...
  \begin{itemize}\setlength\itemsep{2pt}
    \item Driven by FreeRTOS task \texttt{MotorControlTask} with
...
\begin{frame}{Tasks \& schedulability}
  ...
  \begin{tabular}{@{}llcc@{}}
    ...
    \texttt{MotorControlTask}& \(5\)~ms & \texttt{Realtime6} & 2\\
