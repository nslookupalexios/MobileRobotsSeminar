  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.50\textwidth}
      ...
      \begin{description}\setlength\itemsep{2pt}
        \item[\texttt{controller\_manager}] ticks controllers and hardware at a fixed rate.
        \item[Controllers] consume \emph{state interfaces}, produce \emph{command interfaces}\\
              (\texttt{diff\_drive\_controller}, \ldots).
        \item[Hardware components] expose those interfaces to the real device (\texttt{SystemInterface}).
      \end{description}

      \vspace{0.3em}
      \textbf{Why it matters.}\;
      Separates \emph{kinematics} (reusable controller) from
      \emph{hardware access} (your \texttt{SystemInterface}).
    ...
    \column{0.50\textwidth}
      ...
      \begin{tikzpicture}[node distance=4mm and 6mm]
        ...
        \node[msg, right=1mm of dc.east, anchor=west] {cmd~\(d_i\)};
